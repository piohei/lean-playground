...
def env := Lampe.Env.mk [«permute», «SIGMA», «to_le_bytes», «bar», «mtree_recover», «RC», «weird_assert_eq», «rotate_left», «square», «sbox», «sgn0», «to_le_bits», «from_le_bytes», «rl», «as_array», «main», «weird_eq_witness»] [impl_405]